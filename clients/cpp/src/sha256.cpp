...
void Sha256::update(const void* data, std::size_t len) {
    ...
    while (len > 0) {
        const std::size_t take = std::min(kBlockBytes - buffered_, len);
...
Sha256::Digest Sha256::finish() {
    // Padding: 0x80, then zeros, then the length in bits as big-endian 64. The length must land
    // in the final block, so a message that leaves fewer than 9 bytes free needs an extra block.
    buffer_[buffered_++] = 0x80;
    if (buffered_ > kBlockBytes - 8) {
        std::memset(buffer_.data() + buffered_, 0, kBlockBytes - buffered_);
        compress(buffer_.data());
        buffered_ = 0;
    }
    std::memset(buffer_.data() + buffered_, 0, kBlockBytes - 8 - buffered_);

    for (int i = 0; i < 8; ++i) {
        buffer_[kBlockBytes - 1 - i] = static_cast<std::uint8_t>(total_bits_ >> (8 * i));
    }
    compress(buffer_.data());
...
Sha256::Digest Sha256::hash(const void* data, std::size_t len) {
    Sha256 h;
    h.update(data, len);
    return h.finish();
}
...
std::string to_hex(const Sha256::Digest& d) { return to_hex(d.data(), d.size()); }
